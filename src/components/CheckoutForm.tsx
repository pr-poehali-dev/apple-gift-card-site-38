...
const CheckoutForm = ({ selectedCard, onClose, onSuccess }: CheckoutFormProps) => {
      ...
      <Card className="w-full max-w-2xl bg-white rounded-xl overflow-hidden shadow-2xl">
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between mb-8">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center">
                ...
                <div className={`ml-2 text-sm ${s === step ? 'text-black font-medium' : 'text-gray-500'}`}>
                  {s === 1 ? 'Контактные данные' : s === 2 ? 'Способ доставки' : 'Оплата'}
                </div>
                {s < 3 && (
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 2 && (
              <div className="space-y-4">
                <p className="text-gray-600 mb-4">Выберите способ получения подарочной карты:</p>
                <RadioGroup 
                ...
                >
                  <div className="flex items-center space-x-2 rounded-lg border p-4 cursor-pointer hover:bg-gray-50">
                    ...
                    <Label htmlFor="email-delivery" className="flex-1 cursor-pointer">
                      <div className="font-medium">Email</div>
                      <div className="text-sm text-gray-500">Получите код моментально на указанный email</div>
                    </Label>
                    <Icon name="Mail" className="h-5 w-5 text-gray-400" />
                  </div>
                  <div className="flex items-center space-x-2 rounded-lg border p-4 cursor-pointer hover:bg-gray-50">
                    <RadioGroupItem value="sms" id="sms-delivery" />
                    <Label htmlFor="sms-delivery" className="flex-1 cursor-pointer">
                      <div className="font-medium">SMS</div>
                      <div className="text-sm text-gray-500">Получите код моментально на указанный телефон</div>
                    </Label>
                    <Icon name="MessageSquare" className="h-5 w-5 text-gray-400" />
                  </div>
                </RadioGroup>
              </div>
            )}
                ...
                >
                  <div className="flex items-center space-x-2 rounded-lg border p-4 cursor-pointer hover:bg-gray-50">
                    ...
                    <div className="flex space-x-1">
                      <Icon name="CreditCard" className="h-5 w-5 text-gray-400" />
                    </div>
                  </div>
                  <div className="flex items-center space-x-2 rounded-lg border p-4 cursor-pointer hover:bg-gray-50">
                    ...
                    <Label htmlFor="apple-pay" className="flex-1 cursor-pointer">
                      ...
                      <div className="text-sm text-gray-500">Быстрая оплата через Apple Pay</div>
                    </Label>
